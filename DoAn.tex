...
% \usepackage{svg} % Đã comment vì gây lỗi gói transparent trên MiKTeX
...
\usepackage{longtable} % Bảng dài tự ngắt trang (dùng cho danh mục từ viết tắt)
\usepackage{enumitem} % Tùy biến list (\setlist, ...)
...
\usepackage[table]{xcolor} % Tô màu nền cho ô/hàng của bảng (\rowcolor, \cellcolor)
...
\usepackage{setspace}
% NOTE: \usepackage{parskip} makes paragraph spacing non-uniform,
% especially around lists, displays, and headings. Disable it for consistent spacing.
% \usepackage{parskip}
% package content table
\usepackage{tocbasic}

\usepackage[normalem]{ulem} % normalem: giữ \emph in nghiêng (không để ulem biến thành gạch chân); \uline/\dotuline vẫn dùng được
\usepackage{blindtext}
\usepackage{tikz}
\usetikzlibrary{positioning, arrows.meta, fit, backgrounds, calc}
% (multirow + xcolor[table] đã được nạp ở trên, dòng 30-31)
% ===================================================
 


\renewcommand{\bibname}{Danh_sach_tai_lieu_tham_khao}
% Sort citations by order of appearance in the document (not by .bib order)
\usepackage[backend=biber,style=ieee,sorting=none]{biblatex}  %backend=biber is 'better'
...
\addbibresource{\subfix{datn.bib}} % chèn file chứa danh mục tài liệu tham khảo vào

% Co giãn thêm để dòng dài (tên quyển/tạp chí) tự xuống dòng, không tràn lề
\setlength{\emergencystretch}{3em}
% Cho phép URL/DOI tự ngắt dòng (giữ link nhưng không tràn lề phải)
\setcounter{biburlnumpenalty}{7000}   % ngắt được tại chữ số
\setcounter{biburlucpenalty}{7000}    % ngắt được tại chữ HOA
\setcounter{biburllcpenalty}{7000}    % ngắt được tại chữ thường

% --- Chính sách hiển thị link trong danh mục tham khảo ---
% Luôn bỏ ngày truy cập; giữ DOI. Bỏ URL nếu đã có DOI hoặc arXiv (eprint);
% nhưng GIỮ URL khi đó là cách duy nhất để tra cứu (vd. GitHub, phần mềm, trang web).
\AtEveryBibitem{%
  \clearfield{urlyear}\clearfield{urlmonth}\clearfield{urlday}% bỏ "Accessed: ..."
  \ifboolexpr{ test {\iffieldundef{doi}} and test {\iffieldundef{eprint}} }
    {}% không có DOI lẫn eprint -> GIỮ link (GitHub/online-only)
    {\clearfield{url}}% có DOI hoặc eprint -> bỏ link
}

%\include{lstlisting} % Phần này cho phép chèn code và formatting code như C, C++, Python

\makenoidxglossaries % (danh mục từ viết tắt nay là bảng trong Tu_viet_tat.tex, in ở phần đầu quyển)
...
\hypersetup{breaklinks=true} % Cho phép link (vd. tựa bài được gắn link) ngắt dòng, không tràn lề
...
% =================== PHIẾU ĐÁNH GIÁ =================
\newpage
\subfile{Phieu_danh_gia_GVHD} % Phiếu đánh giá - Cán bộ hướng dẫn
\newpage
\subfile{Phieu_danh_gia_GVPB} % Phiếu đánh giá - Cán bộ phản biện
...
\newpage
\input{Tu_viet_tat} % Bảng DANH MỤC TỪ VIẾT TẮT
...
\subfile{Chuong/2_CoSo_TongQuan}

\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{PHƯƠNG PHÁP ĐỀ XUẤT VÀ THIẾT LẬP THỰC NGHIỆM}
\label{chapter:Proposed}
\subfile{Chuong/3_DeXuat_ThietLap}

\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{KẾT QUẢ THỰC NGHIỆM VÀ ĐÁNH GIÁ}
\label{chapter:Results}
\subfile{Chuong/7_Ket_qua}

\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter{THIẾT KẾ VÀ TRIỂN KHAI HỆ THỐNG THEO DÕI fECG THỜI GIAN THỰC}
\label{chapter:System}
\subfile{Chuong/8_HeThong}
